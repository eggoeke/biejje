...
#must be of chain and not flatchain
def plotwalkerlkhd(nwalkers, sampler,sampleStep):
    pl.figure()
    pl.title("Walker's likelyhood over number of iterations")
    pl.xlabel("Number of iterations")
    pl.ylabel("Likelihood")
    for i in range(nwalkers):
        pl.plot([x*sampleStep for x in range(0,nsamples/sampleStep)],np.power(math.e,sampler.lnprobability[i,::sampleStep]))

def plotcorner(nbin,sampler):
    fig = corner.corner(sampler.flatchain[:],bins=nbin, labels=["$x$","$y$"], truths=[0,0])
